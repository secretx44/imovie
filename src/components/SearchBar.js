...
function SearchBar() {
  ...
  const handleSearch = async () => {
    if (searchTerm === '') {
      const { data } = await axios.get('https://api.themoviedb.org/3/movie/popular?api_key=5b0e5846ca807c54fbd77c4694dbbd5f');
      setMovies(data.results);
    } else {
      const { data } = await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=5b0e5846ca807c54fbd77c4694dbbd5f&query=${searchTerm}`);
      setMovies(data.results);
    }
  };
    return (
        <div className="grid grid-cols-1 w-full text-center align-center mx-auto bg-slate-700">
				<div className="flex text-center mx-auto py-4">
				<input
        type="text"
        placeholder="Search movies"
        value={searchTerm}
        onChange={(e) => setSearchTerm(e.target.value)}
				className="border-2 border-cyan-600 outline-none rounded-md h-10"
      />
      <button onClick={handleSearch} className="mx-4 text-white">Search</button>
      </div>
			<ul>
        {Loading ? (
          ...
          <motion.div 
              ...
              className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4 place-items-center text-justify "
					>
            {movies.map((movie) => (
              <li key={movie.id}>
